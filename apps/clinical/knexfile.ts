...
function getDefaultConnectionConfiguration(): Knex.MySql2ConnectionConfig {
  const config: Knex.MySql2ConnectionConfig = {
    database: normalizeString(process.env.DB_NAME_CLINICAL),
...
function getConnectionConfiguration(): IMySQL2Config {
  const config = getDefaultConnectionConfiguration();
...
function knexConfig(): IMySQL2Config {
  const connectionConfig = getConnectionConfiguration();
  return {
    client: 'mysql2',
    ...connectionConfig,
    pool: {
