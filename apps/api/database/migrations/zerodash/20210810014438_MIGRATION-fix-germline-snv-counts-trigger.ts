...
export async function up(knex: Knex): Promise<void> {
  ...
  const trigger = (source_table, update_table, pkey, sid, event) => {
    const condition =
      event === 'UPDATE'
        ? 'NEW.reportable <> OLD.reportable OR NEW.targetable <> OLD.targetable OR NEW.pathclass <> OLD.pathclass'
        : 'NEW.reportable IS NOT NULL OR NEW.targetable IS NOT NULL OR NEW.pathclass IS NOT NULL';
      ...
      THEN
        ...
        VALUES
          ...
          (select count(${sid}) as \`reportable\` from ${source_table}
            where reportable is not null and reportable <> 'Not reportable'
            and ${pkey}=NEW.${pkey}),
