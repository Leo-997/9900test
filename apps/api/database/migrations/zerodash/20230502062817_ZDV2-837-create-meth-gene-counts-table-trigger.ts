...
export async function up(knex: Knex): Promise<void> {
  const triggerName = (table: string, event: 'INSERT' | 'UPDATE') => `${table}_AFTER_${event}`;
  const trigger = (
    sourceTable: string,
    updateTable: string,
    pkeys: string[],
        ...
        THEN
          ...
          VALUES
              (${valuesList},
                ...
                where reportable is not null and reportable <> 'Not Reportable' and reportable <> '0' and ${pkConditions}),
