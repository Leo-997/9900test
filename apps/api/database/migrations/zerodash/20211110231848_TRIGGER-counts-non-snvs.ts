...
export async function up(knex: Knex): Promise<void> {
  ...
  const trigger = (source_table, update_table, pkeys, sid, event) => {
    ...
    const condition =
      event === 'UPDATE'
        ? 'NEW.reportable <> OLD.reportable OR NEW.targetable <> OLD.targetable'
        : 'NEW.reportable IS NOT NULL OR NEW.targetable IS NOT NULL';
          ...
          VALUES
              (${values_list},
                ...
                (select count(distinct cancer_type) as \`cancer_types\`
                  ...
                  on a.${sid} = b.${sid}
                  where ${pk_conditions}),
                (select count(distinct ${keys_noquote}) as \`reported_count\` from ${source_table}
                where reportable is not null and reportable <> 'Not Reportable' and reportable <> '0' and ${pk_conditions}),
                (select count(distinct ${keys_noquote}) as \`targetable_count\` from ${source_table}
