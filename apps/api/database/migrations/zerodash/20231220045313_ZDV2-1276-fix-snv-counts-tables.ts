          ...
          VALUES
              (${valuesList},
                ...
                (select count(distinct ${sid}) as \`c5_count\`
                  from ${sourceTable}
                  where pathclass like 'C5:%' and ${pkConditions}),
                (select count(distinct ${sid}) as \`c4_count\`
                  from ${sourceTable}
                  where pathclass like 'C4:%' and ${pkConditions}),
                (select count(distinct ${sid}) as \`c3_8_count\`
                  from ${sourceTable}
                  where pathclass like 'C3.8:%' and ${pkConditions}),
                (select count(distinct ${sid}) as \`c3_count\`
                  from ${sourceTable}
                  where pathclass like 'C3:%' and ${pkConditions}),
