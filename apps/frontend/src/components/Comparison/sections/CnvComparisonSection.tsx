...
import {
  ...
  IComparisonCnvRow,
  useCnvComparison,
} from '@/hooks/Comparison/useCnvComparison';
...
type CnvRowFilter = 'All' | 'Shared' | 'Unique' | 'Changed';
...
function getStatusClassName(
  status: CnvRowFilter | IComparisonCnvRow['status'],
...
function DifferenceSummary({
  ...
  return (
    <Box className={classes.differenceBlock}>
      {row.differences.map((difference: IComparisonCnvDifference) => (
        <CustomTypography
...
export default function CnvComparisonSection({
  primaryAnalysisSetId,
  comparisonAnalysisSetId,
}: IProps): JSX.Element {
  const classes = useStyles();
  const [filter, setFilter] = useState<CnvRowFilter>('All');
  const {
    loading,
    error,
    primaryIssue,
    ...
    rows,
    counts,
  } = useCnvComparison(primaryAnalysisSetId, comparisonAnalysisSetId);
  ...
  if (loading) {
    content = (
      <Box className={classes.infoCard}>
        <CustomTypography variant="titleRegular" fontWeight="medium">
          Loading CNV comparison
        </CustomTypography>
        <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
          Reportable somatic CNVs are being prepared for both selected samples.
        </CustomTypography>
      </Box>
    );
  } else if (error) {
    ...
    content = (
      <Box className={classes.infoCard}>
        <CustomTypography variant="titleRegular" fontWeight="medium">
          No reportable CNVs found
        </CustomTypography>
        <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
          There are no reportable somatic CNVs to compare for the currently
          selected samples.
        </CustomTypography>
      </Box>
    );
    ...
    content = (
      <Box className={classes.infoCard}>
        ...
        <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
          Try another status filter to see the CNV rows in this comparison.
        </CustomTypography>
      </Box>
    );
  } else {
    ...
    >
      <Box className={classes.header}>
        <Box>
          ...
          <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
            Reportable somatic CNVs are aligned by locus so shared, unique, and
            changed copy-number findings stay visible side by side.
          </CustomTypography>
        </Box>
