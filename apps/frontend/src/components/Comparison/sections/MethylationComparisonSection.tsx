...
import {
  ...
  MethylationSubgroup,
  useMethylationComparison,
} from '@/hooks/Comparison/useMethylationComparison';
...
type MethylationRowFilter = 'All' | 'Shared' | 'Unique' | 'Changed';

const subgroupLabels: MethylationSubgroup[] = [
  'Classifier Results',
  'Prediction / Call',
];
...
function getStatusClassName(
  status: MethylationRowFilter | IComparisonMethylationRow['status'],
...
function MetricsCell({
  ...
  row: IComparisonMethylationRow['primary'];
}): JSX.Element {
...
function DifferenceSummary({
  ...
  return (
    <Box className={classes.differenceBlock}>
      {row.differences.map((difference: IComparisonMethylationDifference) => (
        <CustomTypography
...
export default function MethylationComparisonSection({
  primaryAnalysisSetId,
  comparisonAnalysisSetId,
}: IProps): JSX.Element {
  const classes = useStyles();
  const [filter, setFilter] = useState<MethylationRowFilter>('All');
  const {
    loading,
    error,
    primaryIssue,
    ...
    rows,
    counts,
  } = useMethylationComparison(primaryAnalysisSetId, comparisonAnalysisSetId);
  ...
  const groupedRows = useMemo(() => subgroupLabels.reduce(
    ...
    {} as Record<MethylationSubgroup, IComparisonMethylationRow[]>,
  ), [filteredRows]);

  const filterOptions: Array<{
    label: MethylationRowFilter;
    count: number;
    className: string;
  }> = [
  ...
  if (loading) {
    content = (
      <Box className={classes.infoCard}>
        <CustomTypography variant="titleRegular" fontWeight="medium">
          Loading Methylation comparison
        </CustomTypography>
        <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
          Classifier results and methylation call data are being prepared for both selected samples.
        </CustomTypography>
      </Box>
    );
  } else if (error) {
    content = (
      <Box className={classes.infoCard}>
        <CustomTypography variant="titleRegular" fontWeight="medium">
          Unable to load Methylation comparison
        </CustomTypography>
        <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
          {error}
        </CustomTypography>
    ...
    content = (
      <Box className={classes.infoCard}>
        <CustomTypography variant="titleRegular" fontWeight="medium">
          No reportable Methylation rows found
        </CustomTypography>
        <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
          There are no meaningful methylation findings to compare for the currently
          selected samples.
        </CustomTypography>
      </Box>
    );
    ...
    content = (
      <Box className={classes.infoCard}>
        ...
        <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
          Try another status filter to see the methylation rows in this comparison.
        </CustomTypography>
      </Box>
    );
  } else {
  ...
  return (
    <Box
      ...
      id="comparison-section-methylation"
    >
      <Box className={classes.header}>
        <Box>
          ...
          <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
            Classifier results and methylation call data are aligned so predicted
            group, score, interpretation, and call changes stay easy to compare.
          </CustomTypography>
        </Box>
      ...
      {(primaryIssue || comparisonIssue) && (
        <Box className={classes.issueCard}>
          <Box className={classes.issueBlock}>
            ...
            <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
              {primaryIssue || 'Methylation biosample available.'}
            </CustomTypography>
          </Box>
          <Box className={classes.issueBlock}>
            <CustomTypography variant="label" color={corePalette.grey100}>
            ...
            <CustomTypography variant="bodyRegular" color={corePalette.grey200}>
              {comparisonIssue || 'Methylation biosample available.'}
            </CustomTypography>
          </Box>
        </Box>
      )}
