...
export default function GeneCard({
  ...
  nameWidth,
  searchQuery,
}: IGeneCardProps): JSX.Element {
  const [geneModalOpen, setGeneModalOpen] = useState<boolean>(false);
  ...
  useLayoutEffect(() => {
    const calculateVisibleChips = (): void => {
      const wrapper = chipWrapperRef.current;
      if (wrapper) {
  ...
  const getHighlightedLetters = (geneName: string, query: string): string | JSX.Element[] => {
    if (!query) return geneName;

    const regex = new RegExp(`(${query})`, 'i');
    const letters = geneName.split(regex);

    return letters.map((section) => (
      regex.test(section) ? (
        <span key={`hl-${section}`} style={{ backgroundColor: `${corePalette.green10}` }}>
          {section}
        </span>
      ) : (
        <span key={`dim-${section}`} style={{ color: `${corePalette.grey50}` }}>
          {section}
        </span>
      )
    ));
  ...
  return (
    <>
      <Tooltip
        title={(
          <ToolTipContent>
            {gene.isSomaticGermline && listType === 'somatic' ? (
              <ToolTipTitleWrapper>
                <CustomTypography fontWeight="medium" sx={{ marginRight: '0.3vw' }}>
                  {getHighlightedLetters(gene.gene, searchQuery)}
                </CustomTypography>
                <Dna
                  style={{
                    marginLeft: '0.3vw',
        ...
        enterNextDelay={200}
      >
        <GeneCell onClick={(): void => setGeneModalOpen(true)}>
          {gene.isSomaticGermline && listType === 'somatic' ? (
          ...
          <ChipsAndOverflowWrapper nameWidth={nameWidth}>
            <ChipsWrapper ref={chipWrapperRef}>
      ...
      <GeneModal
        open={geneModalOpen}
        onClose={(): void => setGeneModalOpen(false)}
        gene={gene}
        listType={listType}
      />
    </>
