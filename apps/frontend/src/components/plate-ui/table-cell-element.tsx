...
import { useRTETable } from './Contexts/RTETableContext';
...
>(({
  children, className, isHeader, style, ...props
}, ref) => {
  const { api } = useEditorPlugin(TablePlugin);
  const readOnly = useReadOnly();
  const element = props.element as TTableCellElement;
  const { flexibleTableWidth } = useRTETable();
  ...
  return (
    <PlateElement
      key={`table-cell-${element.id}-${flexibleTableWidth}`}
      ref={ref}
      as={isHeader ? 'th' : 'td'}
      className={cn(
        className,
      ...
      style={
        {
          ...
          maxWidth: flexibleTableWidth ? 'auto' : width || 240,
          minWidth: flexibleTableWidth ? 'auto' : width || 120,
          ...style,
