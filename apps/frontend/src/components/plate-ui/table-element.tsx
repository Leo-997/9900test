  ...
  ({ children, ...props }, ref) => {
    ...
    return (
      <Popover open={canMerge || canSplit || collapsed} modal={false}>
        ...
        >
          <Toolbar
            className="bg-white flex scrollbar-hide w-auto max-w-[80vw] flex-row overflow-x-auto rounded-md border p-1 shadow-md print:hidden opacity-100"
            contentEditable={false}
          >
            <ToolbarGroup>
              {canMerge && (
                <ToolbarButton
                  onClick={(): any => tf.table.merge()}
